...
# очищаем таблицы с исходными данными
def truncate_tables (ti):
    ...
    cur = conn.cursor()
    cur.execute("""
    truncate table stg.api_deliveries_raw;
    truncate table stg.api_restaurants_raw;
    truncate table stg.api_couriers_raw;""")
    conn.commit()
    cur.close()
    conn.close()
...
def load_deliveries_to_stg (ti, url=url, headers=headers):
    ...
    while True:
      ...
      for i in range(len(data_json)): #
          query = f"INSERT INTO stg.api_{table_name}(json_content) VALUES ('{json.dumps(data_json[i], ensure_ascii=False)}'::json)"
          cur.execute(query)
      offset += len(data_json)
...
dag = DAG(
   dag_id = 'sprint5_project_dag',  
    schedule_interval='0 * * * *',  # Задаем расписание выполнения дага - каждый час.
...
t_truncate_tables = PythonOperator(task_id='truncate_tables',
...
t_truncate_tables >> t_load_couriers_to_stg >> t_load_restaurants_to_stg >> t_load_deliveries_to_stg >> t_dds_cdm_insert
